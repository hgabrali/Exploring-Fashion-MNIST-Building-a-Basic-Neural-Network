...
<img width="802" height="527" alt="image" src="https://github.com/user-attachments/assets/88101b65-ad14-440d-ba05-1de077e938bc" />

* Softmax Aktivasyon Fonksiyonu'nun çalışma mantığını görselleştirir. Özellikle çok sınıflı sınıflandırma (multiclass classification) problemlerinde, modelin ham çıktılarını anlamlı olasılıklara nasıl dönüştürdüğünü gösterir.
...
### 🧠 Softmax Activation Function Analysis

The Softmax activation function is the mathematical engine behind the classification head of our model. It serves as the final bridge between raw numerical calculations and human-interpretable probability distributions.

---

#### 🔬 1. Logit-to-Probability Mapping (Logit-Olasılık Dönüşümü)

The raw numerical values generated by the **Dense layer** (yoğun katman) are referred to as **Logits**. These values can be any real number, making them difficult to interpret directly as classification results.

* **Compression:** The Softmax function compresses these complex logits into **Probability values** strictly ranging between $0.0$ and $1.0$.
* **Input Logit ($z_1$):** On a diagnostic graph, the x-axis represents the raw score generated for the first class.
* **Output Probability:** The y-axis indicates the model's level of **Confidence** (güven) in that specific class.



---

#### ⚖️ 2. Sum to Unity Rule (Toplamın Bir Olma Kuralı)

A fundamental characteristic of the Softmax function is its constraint that the sum of probabilities for all output classes must always equal **$1$** (representing $100\%$).

* **Competitive Dynamics:** As the probability for Class 1 rises (the green line on a standard graph), the probability for Class 2 (the orange line) decreases proportionally.
* **Inter-class Competition:** This illustrates how classes **compete** (birbiriyle yarışma) with each other; gaining confidence in one class inherently reduces the likelihood of the others.

---

#### 🚧 3. Decision Boundary and Uncertainty (Karar Eşiği ve Belirsizlik)

The behavior of the function at its center point ($x=0$) provides critical insights into the model's decision-making process:

* **Indecision Point:** If the logit values for all classes are equal (e.g., $z_1 = 0$ and $z_2 = 0$), Softmax assigns an equal probability (e.g., $50\%$ or $0.5$ in a two-class scenario) to each. This represents the region where the model is **Indecisive** (kararsız).
* **Certainty Scaling:** As the $z_1$ value increases in the positive direction, the model's **Decision Confidence** grows, and the probability curve shifts asymptotically toward $1.0$.
...
| English Term | Turkish Equivalent | Description |
| :--- | :--- | :--- |
| **Multiclass Classification** | Çok Sınıflı Sınıflandırma | Scenarios involving more than 2 distinct classes (e.g., the 10 categories in Fashion MNIST). |
| **Logit** | Logit (Ham Skor) | The raw, unnormalized output of the model before processing by an activation function. |
| **Probability Distribution** | Olasılık Dağılımı | A state where the sum of all individual class outputs equals exactly $1$. |
| **Exponential Growth** | Üssel Büyüme | Softmax utilizes $e^z$ to ensure that larger logits become significantly more dominant in the final probability. |
| **Decision Confidence** | Karar Güveni | A metric indicating how close a probability value is to $1$, representing the model's certainty. |
...
### ↗️ Batching: Optimizing Data Processing

To enhance computational efficiency and memory management during the training phase, we implement a strategy known as **Batching**. Instead of feeding the entire dataset into the model simultaneously, we partition the data into smaller, manageable subsets.

---

#### ⚙️ The Batching Mechanism

In this project, we have defined a **Batch Size of 32**. This means that the model will process the data in a sequential, iterative manner:

1.  **Iterative Input:** The model receives the first batch (32 images), calculates the loss, and updates weights.
2.  **Sequential Processing:** It then moves to the second batch, then the third, and continues this cycle until all data has been processed.
3.  **Completion:** The process repeats until the entire training set has been exhausted for the current epoch.



---

### 📈 Mathematical Breakdown of Batches

The total number of iterations required to cover the entire dataset is determined by the relationship between the total sample count and the chosen batch size.

* **Total Training Images:** 60,000
* **Batch Size:** 32
* **Total Batches:** 1,875

$$Total Batches = \frac{Total Samples}{Batch Size} = \frac{60,000}{32} = 1,875$$

### 🛠️ Technical Importance of Batching

* **Memory Efficiency:** By only loading 32 images into the GPU/CPU memory at a time, we prevent memory overflow (Out of Memory errors), especially when working with high-resolution data or complex architectures.
* **Stochastic Gradient Descent (SGD):** Processing data in batches introduces a level of "noise" in the gradient calculation, which often helps the model escape local minima and find more generalized global optima.
...
* **Generalization Performance (Genelleme Performansı):** The primary objective is to evaluate how effectively the neural network classifies images from the **Test Set (10,000 samples)**—data that was never presented to the model during optimization.
* **Accuracy Metric (Doğruluk Metriği):** Based on the evaluation results, our current baseline model achieves a **Test Accuracy of $88.63\%$**.
* **Statistical Translation:** This metric implies that approximately **8,863 out of 10,000** test images were identified correctly by the model.
* **Model Complexity:** The architecture consists of **101,770 trainable parameters**, occupying roughly **397.54 KB** of memory.
...
To bridge the remaining gap in accuracy, the project will transition from a basic ANN to a more sophisticated architecture:

| Current Architecture | Target Architecture | Reason for Transition |
| :--- | :--- | :--- |
| **Basic ANN (Dense)** | **CNN (Convolutional)** | CNNs utilize spatial filters to detect edges and local patterns that basic dense layers overlook. |

---

## 📊 Confusion Matrix Diagnostic Analysis

The **Confusion Matrix (Karmaşıklık Matrisi)** is a critical diagnostic tool that provides a granular breakdown of the model's predictive performance.

### 🔍 What Does This Matrix Tell Us?
The visualization reveals specific patterns in the model's decision-making logic:

* **✅ Diagonal Dominance (True Positives):** The high numerical values along the diagonal represent **True Positives (Doğru Tahminler)**. The model is highly effective at identifying distinct silhouettes like **Trousers (982)**, **Ankle boots (971)**, and **Bags (962)**.
* **⚠️ Structural Similarity Confusion:** The matrix reveals significant misclassification between items with similar silhouettes, confirming our **High Inter-class Similarity** hypothesis:
    * **Shirt vs. T-shirt/top:** 88 "Shirts" were predicted as "T-shirt/top," while 135 "T-shirt/tops" were confused with "Shirts".
    * **Pullover vs. Coat:** 103 "Pullovers" were misclassified as "Coats".
* **📉 Weak Feature Extraction:** The persistent confusion between **"Shirt," "T-shirt," and "Coat"** suggests that a simple ANN struggles to extract fine-grained details (like buttons or collars) in a flattened $28 \times 28$ format.
...
Bu grafik bize modelin **"Pantolon"** ve **"Bot"** gibi belirgin şekilleri çok iyi tanıdığını ($>97\%$), ancak **"Gömlek"** ve **"Tişört"** gibi yapısal olarak benzeyen ürünlerde hala zorlandığını söyler. Bu durum, mekansal özellikleri (**spatial features**) daha iyi yakalayan **CNN mimarisine** geçiş yapmamız gerektiğini kanıtlar.
